<?php
include "koneksi.php";
?>

<!DOCTYPE html>
<html lang="en">

<head>
     <meta charset="UTF-8">
     <meta name="viewport" content="width=device-width, initial-scale=1.0">
     <title>Document</title>
     <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet"
          integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous">
     <link rel="stylesheet" href="style.css">

     <!-- bootstrap icon -->
     <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.min.css">

</head>


<body>
     <nav class="navbar navbar-expand-lg bg-body-tertiary p-4">
          <div class="container">
               <a class="navbar-brand" href="#">
                    <img src="./img/paw.png" alt="Logo" width="30" height="24" class="d-inline-block align-text-top">
                    Doggy Store
               </a>

               <button class="navbar-toggler" type="button" data-bs-toggle="collapse"
                    data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent"
                    aria-expanded="false" aria-label="Toggle navigation">
                    <span class="navbar-toggler-icon"></span>
               </button>
               <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                         <li class="nav-item">
                              <a class="nav-link hover-effect px-3 py-2 rounded-4" aria-current="page"
                                   href="#home"><span><img
                                             src="data:image/png;base64," /></span>
                                   Home</a>
                         </li>
                         <li class="nav-item">
                              <a class="nav-link hover-effect px-3 py-2 rounded-4" href="#petshop"><span><img
                                             src="data:image/png;base64," /></span>Pet
                                   shop</a>
                         </li>
                         <li class="nav-item">
                              <a class="nav-link hover-effect px-3 py-2 rounded-4" href="#gallery"><span><img
                                             src="data:image/png;base64," /></span>
                                   Gallery</a>
                         </li>
                         <li class="nav-item">
                              <a class="nav-link hover-effect px-3 py-2 rounded-4" href="login.php"><span><img
                                             src="data:image/png;base64," /></span>
                                   Login</a>
                         </li>
                    </ul>
                    <form class="d-flex" role="search">
                         <input class="form-control me-2" type="search" placeholder="Search" aria-label="Search">
                         <button class="btn btn-outline-success" type="submit">Search</button>
                    </form>
               </div>
          </div>
     </nav>

     <!-- Jumbotron / Hero -->
     <div class="p-5 mb-4 bg-body-tertiary rounded-3 bg-primary-subtle" id="home">
          <div class="container py-5">
               <div class="d-flex align-items-center justify-content-around flex-wrap">
                    <div class="">
                         <h1 class="display-5 fw-bold">Selamat Datang di Doggy Store</h1>
                         <p class="col-md-8 fs-4">Temukan anjing pilihan anda disini</p>
                         <button class="btn btn-primary btn-md" type="button">Explore</button>
                    </div>
                    <div class="">
                         <img src="./img/dogicon.png" alt="" width="200" height="200" class="d-none d-md-block">
                         <img src="./img/dogicon.png" alt="" width="100" height="100" class="d-md-none mt-5">
                    </div>
               </div>
          </div>
     </div>

     <!-- pet shop -->
     <div class="mt-5" id="petshop">
          <div class="container">
               <div>
                    <h1 class="py-2 px-3 rounded-5 bg-primary-subtle d-inline-block">Pet shop</h1>
                    <p>Berikut adalah daftar hewan anjing yang tersedia di platform kami</p>
               </div>
               <div class="mt-5 d-flex justify-content-center flex-wrap gap-5">
                    <?php
                    $sql = "SELECT * FROM dogshop ORDER BY id DESC";
                    $hasil = $conn->query($sql);

                    while ($row = $hasil->fetch_assoc()) {
                         ?>
                         <div class="card" style="width: 18rem; ">
                              <img src="img/<?= $row["gambar"] ?>" class="card-img-top image-ratio" alt="gambar anjing">
                              <div class="card-body">
                                   <h5 class="card-title"><?= $row["nama_anjing"] ?></h5>
                                   <div class="row mt-3">
                                        <p class="col"><span><i class="bi bi-calendar-day"></i></span>
                                             <?= $row["tgl_lahir"] ?></p>
                                        <p class="col"><span><i class="bi bi-cake2"></i></span> <?= $row["umur"] ?></p>
                                   </div>
                                   <div class="row">
                                        <p class="col"><strong>Rp</strong> <?= $row["harga"] ?></p>
                                        <p class="col"><?= $row["jenis_kelamin"] ?></p>
                                   </div>
                                   <div class="row">
                                        <p class="col"><span><i class="bi bi-shop"></i></span>
                                             <?= $row["nama_toko_peternak"] ?></p>
                                        <p class="col"><span><i class="bi bi-geo-alt-fill"></i></span>
                                             <?= $row["kota_peternak"] ?></p>
                                   </div>
                              </div>
                         </div>
                         <?php
                    }
                    ?>
               </div>
          </div>
     </div>

     <!-- Gallery -->
     <div class="" id="gallery">
          <div class="container mt-5 pt-5 pb-5">
               <div>
                    <h1 class="py-2 px-3 bg-primary-subtle d-inline-block rounded-5">Gallery</h1>
                    <p>Berikut adalah daftar dokumentasi pengiriman hewan saat tiba di lokasi pelanggan kami</p>
               </div>
               <div class="mt-5 d-flex justify-content-center flex-wrap gap-5">
                    <?php
                    $sql = "SELECT * FROM gallery ORDER BY id DESC";
                    $hasil = $conn->query($sql);
                    if ($hasil) {
                         while ($row = $hasil->fetch_assoc()) {
                              ?>
                              <div class="card" style="width: 18rem;">
                                   <img src="img/<?= $row["gambar"] ?>" class="card-img-top image-ratio" alt="...">
                                   <div class="card-body">
                                        <p class="card-text"><?= $row["deskripsi"] ?></p>
                                   </div>
                              </div>
                              <?php
                         }
                    }
                    ?>
               </div>
          </div>
     </div>

     <footer class="bg-primary-subtle mt-5 p-5">
          <div class="container">
               <div>
                    <div>
                         <h2 class="" href="#">
                              <img src="./img/paw.png" alt="Logo" width="50" height="39"
                                   class="d-inline-block align-text-top">
                              Doggy Store
                         </h2>
                         <p>Platform khusus penjualan hewan anjing</p>
                    </div>
                    <div class="d-flex justify-content-between flex-wrap">
                         <div class="mt-4 ">
                              <p><span><i class="bi bi-calendar-day"></i></span> Hari Kerja</p>
                              <p>Senin - Jumat</p>
                              <p><span><i class="bi bi-clock"></i></span> Jam Operasional</p>
                              <p><strong>09:00 - 21:00</strong></p>
                         </div>
                         <div class="mt-4">
                              <p><span><i class="bi bi-building"></i></span> Alamat Kantor</p>
                              <p>Jl. Merdeka No. 21, Semarang Timur, Rejosari, Kota Semarang</p>
                              <p><span><i class="bi bi-telephone-fill"></i></span> No Handphone / WA</p>
                              <p>081136114811</p>
                         </div>
                         <div class="mt-4">
                              <p><span><i class="bi bi-envelope"></i></span> Email</p>
                              <p>doggystore@gmail.com</p>
                              <p><span><i class="bi bi-instagram"></i></span> instagram</p>
                              <p>doggystoreofficial</p>
                         </div>
                    </div>
               </div>
          </div>
     </footer>

     <div class="">
          <div class="container">
               <div class="text-center">
                    <p class="p-4 mb-0">Copyright © 2025 Ferdynand Ergy Pramudani. All rights
                         reserved
                    </p>
               </div>
          </div>
     </div>



     <!-- Boxicons -->
     <script src="https://unpkg.com/boxicons@2.1.4/dist/boxicons.js"></script>

     <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"
          integrity="sha384-YvpcrYf0tY3lHB60NNkmXc5s9fDVZLESaAA55NDzOxhy9GkcIdslK1eN7N6jIeHz"
          crossorigin="anonymous"></script>
</body>

</html>